...
void myInitParticles() {
	ifstream myFile;
	std::string word;
	myFile.open("particles.txt");
	if (myFile.is_open()) 
	{
		while (myFile >> word) 
		{
			if (word == "Particle") 
			{
				double x, y, dx, dy, r;
				bool a;
				myFile >> x >> y >> dx >> dy >> r>> a;
				Particle *p1 = new Particle(x, y, dx, dy, r, a);
				PS.AddParticle(p1);
			}
			else if (word == "Spring") {
				int p1, p2;
				double  springConstant, dampingConstant, restLength;
				myFile >> p1 >>  p2 >> springConstant >> dampingConstant >> restLength;
				PS.AddForce(new SpringForce(PS.GetParticle(p1), PS.GetParticle(p2), springConstant, dampingConstant, restLength));
			}
			else if (word == "Drag") {
				double friction;
				myFile >> friction;
				PS.AddForce(new DragForce(friction, &PS));
			}
			else if (word == "Gravity") {
				double a, b;
				myFile >> a >> b;
				double gravity[DIM] = { a, b };
